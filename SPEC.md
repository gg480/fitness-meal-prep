...
**动作指引动图库（v3.2 增量）**：`frontend/src/action-guides.js` 提供 `ACTION_GUIDES`，每个动作 key 挂 `gif`（ExerciseDB `static.exercisedb.dev` 免费非商用 180p 动图，vite 通过 `assetsInclude` 打包进 bundle）+ `tips`（3 条中文要点，基于英文 instructions + 器械适配的二次创作，突出标准姿态+呼吸；无图动作降级为文字）。素材采集脚本 `scripts/fetch_actions.py`（走本机代理 7897），来源需在数据文件内署名 ExerciseDB。三展示点：下一练列表缩略动图按钮 / 动作名按钮 / 训练中「指引▶」按钮，均打开 `ActionModal` 弹窗（大图 + 要点列表，无图显示「暂无动图 · 按要领执行」）。
...
- **饮食侧**：今日页进入「历史日期模式」（只读快照 + 可补：餐份数 / 日类型 / 打卡标记），**只补记录、绝不回溯重算**——`per_snap` / `meals_log` 是历史事实层（打卡时刻已冻结），读时派生层随补录更新（这正是补打卡的意义：让记录更真实）。已实现：`day-logs` PUT 支持任意历史 `date`（whey 必填 0-6 校验）；今日页原硬编码「今天」，已加历史模式（`histDate` 非空整页切到「快照 + 补录」视图，`saveBackfill` 以「默认空档 + 原记录覆盖」为底只覆盖 meals/checkedIn/dayType 三字段）。
...
| v3.2.0 | 动作指引动图库（ExerciseDB 免费非商用动图打进 bundle + 中文要点；训练前列表缩略图 / 训练中「指引▶」/ 弹窗大图+要点） | 同上 + 三展示点真机实测，Console 零错误 |
| P2 冻结 | 晨脉趋势、力量线、语音（观察门满 4 周再议）、deload 自动化、课表自定义、free-exercise-db 全量导入 | — |
